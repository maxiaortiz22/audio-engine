...
std::tuple<std::vector<double>, std::vector<double>> iirfilter(int N, 
    ...
    Wn : vector
        ...
        high, eg: <6000, 12000>
    btype : {'FilterType::Lowpass', 'FilterType::Highpass', 'FilterType::Bandpass', 'FilterType::Bandstop'}
    analog : bool
        When True, return an analog filter, otherwise a digital filter is
        returned.
    output : {'ba', 'zpk'}
    ...
    */
    ...
    if (btype == FilterType::Lowpass || btype == FilterType::Highpass){

        if (btype == FilterType::Lowpass){
        ...
        if (btype == FilterType::Bandpass || btype == FilterType::Bandstop){

            double bw = warped[1] - warped[0];
            double wo = std::sqrt(warped[0] * warped[1]);

            if (btype == FilterType::Bandpass){
...
std::vector<float> filter_signal(std::vector<float> signal, std::vector<double> b, std::vector<double> a) {

    int signal_size = signal.size();
    std::vector<float> filtered_signal(signal_size);
    ...
    for (int n = 0; n < signal_size; n++){
        ...
        for (int i = 1; i < b.size(); i++){
            if (n>=i){
                filtered_signal[n] += b[i] * signal[n-i];
            }
            else{
                break;
            }
        ...
        for (int i = 1; i < a.size(); i++){
            if (n>0 && n>=i){
                filtered_signal[n] -= a[i]*filtered_signal[n-i];
            }
            else{
                break;
            }
...
int main()
{
    ...
    int N = 1;                    //Orden del filtro
    std::vector<double> fc{20}; //Frecuencia de corte
    std::string btype = FilterType::Lowpass; //Tipo de filtro
