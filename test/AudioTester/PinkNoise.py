...
class PinkeNoiseTest(NoiseTester):

    def __init__(self, sr, buffer):
        super().__init__(sr, buffer)
        audio_engine.noise_generator_setValue(audio_engine.NoiseGeneratorParam.NSG_NOISE_TYPE, audio_engine.NoiseType.PINK_NOISE)

    def test(self):
        bar = IncrementalBar('Pink Noise test', max = 2)
